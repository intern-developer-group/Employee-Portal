...
function insert_data($con)
{
    ...
    if ($emailcount > 0) { ?>
        ...
        $query1 = "INSERT INTO user (user_name,user_email,user_password,user_type,monthly_salary,daily_salary) VALUES(?,?,?,?,?,?)";
        if ($stmt = mysqli_prepare($con, $query1)) {
            mysqli_stmt_bind_param($stmt, 'sssiii', $name1, $email1, $password1, $usertype, $salary, $dailysalary);
